...
namespace Hathor.Beatport.Lib
{
    public class Beatport
    {
        ...
        {
            HtmlDocument htmlDocument = await HtmlFromUriPath(uriPath);
        ...
        private async Task<HtmlDocument> HtmlFromUriPath(string uriPath)
        {
            IFlurlRequest request = _flurlClient.Request(uriPath);
